...
export type OZNote = {
	...
	displayName: string;
	path: string;
	hashtagPosition?: number; // Номер строки с хештегом (начиная с 0)
};
...
export const hashtagToOZItem = (params: { note: TFile; hashtagText: string; hashtagMatch: string; hashtagPosition?: number }): OZItem => {
	...
	return {
		...
		path: params.note.path,
		hashtagPosition: params.hashtagPosition,
	};
};
